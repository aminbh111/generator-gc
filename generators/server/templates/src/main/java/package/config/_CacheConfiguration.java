...
public class CacheConfiguration {
    ...
    public CacheManager cacheManager(GreencodeProperties greencodeProperties) {
        log.debug("Starting Ehcache");
        cacheManager = net.sf.ehcache.CacheManager.create();
        cacheManager.getConfiguration().setMaxBytesLocalHeap(greencodeProperties.getCache().getEhcache().getMaxBytesLocalHeap());
        ...
        for (EntityType<?> entity : entities) {
            ...
            if (cache != null) {
                cache.getCacheConfiguration().setTimeToLiveSeconds(greencodeProperties.getCache().getTimeToLiveSeconds());
    ...
    @Bean
    public HazelcastInstance hazelcastInstance(GreencodeProperties greencodeProperties) {
        ...
        config.getMapConfigs().put("<%=packageName%>.domain.*", initializeDomainMapConfig(greencodeProperties));<% } %><% if (clusteredHttpSession == 'hazelcast') { %>
        config.getMapConfigs().put("clustered-http-sessions", initializeClusteredSession(greencodeProperties));<% } %>
    ...
    private MapConfig initializeDomainMapConfig(GreencodeProperties greencodeProperties) {
        MapConfig mapConfig = new MapConfig();

        mapConfig.setTimeToLiveSeconds(greencodeProperties.getCache().getTimeToLiveSeconds());
        return mapConfig;
    }
    ...
    */
    ...
    private MapConfig initializeClusteredSession(GreencodeProperties greencodeProperties) {
        MapConfig mapConfig = new MapConfig();

        mapConfig.setBackupCount(greencodeProperties.getCache().getHazelcast().getBackupCount());
        mapConfig.setTimeToLiveSeconds(greencodeProperties.getCache().getTimeToLiveSeconds());
        return mapConfig;
    }<% } %><% } %><% if (clusteredHttpSession == 'hazelcast') { %>
