...
module.exports = EntityGenerator.extend({
    ...
    initializing: {
        getConfig: function (args) {
            ...
            this.greencodeConfigDirectory = '.greencode';
            this.mainClass = this.getMainClassName();

            this.filename = this.greencodeConfigDirectory + '/' + this.entityNameCapitalized + '.json';
        ...
        askForMicroserviceJson: function(){
            ...
            this.prompt(prompts, function(props) {
                if (props.useMicroserviceJson) {
                    ...
                    this.fromPath = this.microservicePath + '/' + this.greencodeConfigDirectory + '/' + this.entityNameCapitalized + '.json';
                    this.useConfigurationFile = true;
        ...
        validateFile: function() {
            ...
            for (var idx in this.fields) {
                ...
                if (_.isUndefined(field.fieldName)) {
                    this.env.error(chalk.red('ERROR fieldName is missing in .greencode/' + this.name + '.json for field ' + JSON.stringify(field, null, 4)));
                }

                if (_.isUndefined(field.fieldType)) {
                    this.env.error(chalk.red('ERROR fieldType is missing in .greencode/' + this.name + '.json for field ' + JSON.stringify(field, null, 4)));
                }

                if (!_.isUndefined(field.fieldValidateRules)) {
                    if (!_.isArray(field.fieldValidateRules)) {
                        this.env.error(chalk.red('ERROR fieldValidateRules is not an array in .greencode/' + this.name + '.json for field ' + JSON.stringify(field, null, 4)));
                    }
                    for (var idxRules in field.fieldValidateRules) {
                        ...
                        if (!_.includes(SUPPORTED_VALIDATION_RULES, fieldValidateRule)) {
                            this.env.error(chalk.red('ERROR fieldValidateRules contains unknown validation rule ' + fieldValidateRule + ' in .greencode/' + this.name + '.json for field ' + JSON.stringify(field, null, 4) + ' [supported validation rules ' + SUPPORTED_VALIDATION_RULES + ']'));
                        }
                    }
                    if (_.includes(field.fieldValidateRules, 'max') && _.isUndefined(field.fieldValidateRulesMax)) {
                        this.env.error(chalk.red('ERROR fieldValidateRulesMax is missing in .greencode/' + this.name + '.json for field ' + JSON.stringify(field, null, 4)));
                    }
                    if (_.includes(field.fieldValidateRules, 'min') && _.isUndefined(field.fieldValidateRulesMin)) {
                        this.env.error(chalk.red('ERROR fieldValidateRulesMin is missing in .greencode/' + this.name + '.json for field ' + JSON.stringify(field, null, 4)));
                    }
                    if (_.includes(field.fieldValidateRules, 'maxlength') && _.isUndefined(field.fieldValidateRulesMaxlength)) {
                        this.env.error(chalk.red('ERROR fieldValidateRulesMaxlength is missing in .greencode/' + this.name + '.json for field ' + JSON.stringify(field, null, 4)));
                    }
                    if (_.includes(field.fieldValidateRules, 'minlength') && _.isUndefined(field.fieldValidateRulesMinlength)) {
                        this.env.error(chalk.red('ERROR fieldValidateRulesMinlength is missing in .greencode/' + this.name + '.json for field ' + JSON.stringify(field, null, 4)));
                    }
                    if (_.includes(field.fieldValidateRules, 'maxbytes') && _.isUndefined(field.fieldValidateRulesMaxbytes)) {
                        this.env.error(chalk.red('ERROR fieldValidateRulesMaxbytes is missing in .greencode/' + this.name + '.json for field ' + JSON.stringify(field, null, 4)));
                    }
                    if (_.includes(field.fieldValidateRules, 'minbytes') && _.isUndefined(field.fieldValidateRulesMinbytes)) {
                        this.env.error(chalk.red('ERROR fieldValidateRulesMinbytes is missing in .greencode/' + this.name + '.json for field ' + JSON.stringify(field, null, 4)));
                    }
                    if (_.includes(field.fieldValidateRules, 'pattern') && _.isUndefined(field.fieldValidateRulesPattern)) {
                        this.env.error(chalk.red('ERROR fieldValidateRulesPattern is missing in .greencode/' + this.name + '.json for field ' + JSON.stringify(field, null, 4)));
                    }
                }
            }
            ...
            for (var idx in this.relationships) {
                ...
                if (_.isUndefined(relationship.relationshipName)) {
                    ...
                    this.log(chalk.yellow('WARNING relationshipName is missing in .greencode/' + this.name + '.json for relationship ' + JSON.stringify(relationship, null, 4) + ', using ' + relationship.otherEntityName + ' as fallback'));
                }

                if (_.isUndefined(relationship.otherEntityName)) {
                    this.env.error(chalk.red('ERROR otherEntityName is missing in .greencode/' + this.name + '.json for relationship ' + JSON.stringify(relationship, null, 4)));
                }
                ...
                if (_.isUndefined(relationship.otherEntityRelationshipName)
                    ...
                    this.log(chalk.yellow('WARNING otherEntityRelationshipName is missing in .greencode/' + this.name + '.json for relationship ' + JSON.stringify(relationship, null, 4) + ', using ' + _.lowerFirst(this.name) + ' as fallback'));
                }
                ...
                if (_.isUndefined(relationship.otherEntityField)
                    ...
                    this.log(chalk.yellow('WARNING otherEntityField is missing in .greencode/' + this.name + '.json for relationship ' + JSON.stringify(relationship, null, 4) + ', using id as fallback'));
                    relationship.otherEntityField = 'id';
                }

                if (_.isUndefined(relationship.relationshipType)) {
                    this.env.error(chalk.red('ERROR relationshipType is missing in .greencode/' + this.name + '.json for relationship ' + JSON.stringify(relationship, null, 4)));
                }
                ...
                if (_.isUndefined(relationship.ownerSide)
                    ...
                    this.env.error(chalk.red('ERROR ownerSide is missing in .greencode/' + this.name + '.json for relationship ' + JSON.stringify(relationship, null, 4)));
                }
            }
            ...
            if (_.isUndefined(this.changelogDate)
                ...
                this.log(chalk.yellow('WARNING changelogDate is missing in .greencode/' + this.name + '.json, using ' + currentDate + ' as fallback'));
                this.changelogDate = currentDate;
            }
            if (_.isUndefined(this.dto)) {
                this.log(chalk.yellow('WARNING dto is missing in .greencode/' + this.name + '.json, using no as fallback'));
                this.dto = 'no';
            }
            if (_.isUndefined(this.service)) {
                this.log(chalk.yellow('WARNING service is missing in .greencode/' + this.name + '.json, using no as fallback'));
                this.service = 'no';
            }
            if (_.isUndefined(this.pagination)) {
                if (databaseType == 'sql' || databaseType == 'mongodb') {
                    this.log(chalk.yellow('WARNING pagination is missing in .greencode/' + this.name + '.json, using no as fallback'));
                    this.pagination = 'no';
                } else {
                    this.pagination = 'no';
                }
    ...
    writing : {
        saveRemoteEntityPath: function() {
            ...
            this.copy(this.microservicePath + '/' + this.greencodeConfigDirectory + '/' + this.entityNameCapitalized + '.json', this.destinationPath(this.greencodeConfigDirectory + '/' + this.entityNameCapitalized + '.json'));
        },
        ...
        afterRunHook: function () {
            try {
                ...
                if (modules.length > 0) {
                    ...
                    var entityConfig = {
                        greencodeConfigDirectory: this.greencodeConfigDirectory,
