...
const GREENCODE_CONFIG_DIR = ".greencode";
const MODULES_HOOK_FILE = GREENCODE_CONFIG_DIR + '/modules/jhi-hooks.json';
...
Generator.prototype.addMessageformatLocaleToBowerOverride = function (language) {
    ...
    try {
        greencodeUtils.rewriteJSONFile(fullPath, function (jsonObj) {
...
Generator.prototype.addElementToMenu = function (routerName, glyphiconName, enableTranslation) {
    try {
        ...
        greencodeUtils.rewriteFile({
            file: fullPath,
            needle: 'greencode-needle-add-element-to-menu',
            splicable: [
        ...
        this.log(chalk.yellow('\nUnable to find ') + fullPath + chalk.yellow(' or missing required greencode-needle. Reference to ') + routerName + ' ' + chalk.yellow('not added to menu.\n'));
    }
};
...
Generator.prototype.addElementToAdminMenu = function (routerName, glyphiconName, enableTranslation) {
    try {
        ...
        greencodeUtils.rewriteFile({
            file: fullPath,
            needle: 'greencode-needle-add-element-to-admin-menu',
            splicable: [
                '<li ui-sref-active="active" ><a ui-sref="' + routerName + '" data-toggle="collapse" data-target=".navbar-collapse.in"><span class="glyphicon glyphicon-' + glyphiconName + '"></span>\n' +
        ...
        this.log(chalk.yellow('\nUnable to find ') + fullPath + chalk.yellow(' or missing required greencode-needle. Reference to ') + routerName + ' ' + chalk.yellow('not added to admin menu.\n'));
    }
};
...
Generator.prototype.addEntityToMenu = function (routerName, enableTranslation) {
    try {
        ...
        greencodeUtils.rewriteFile({
            file: fullPath,
            needle: 'greencode-needle-add-entity-to-menu',
            splicable: [
...
Generator.prototype.addElementTranslationKey = function (key, value, language) {
    ...
    try {
        greencodeUtils.rewriteFile({
            file: fullPath,
            needle: 'greencode-needle-menu-add-element',
            splicable: [
                '"' + key + '": "' + value + '",'
            ]
        }, this);
    } catch (e) {
        this.log(chalk.yellow('\nUnable to find ') + fullPath + chalk.yellow(' or missing required greencode-needle. Reference to ') + language + chalk.yellow(' not added as a new entity in the menu.\n'));
    }
};
...
Generator.prototype.addAdminElementTranslationKey = function (key, value, language) {
    ...
    try {
        greencodeUtils.rewriteFile({
            file: fullPath,
            needle: 'greencode-needle-menu-add-admin-element',
            splicable: [
                '"' + key + '": "' + value + '",'
            ]
        }, this);
    } catch (e) {
        this.log(chalk.yellow('\nUnable to find ') + fullPath + chalk.yellow(' or missing required greencode-needle. Reference to ') + language + chalk.yellow(' not added as a new entry in the admin menu.\n'));
    }
};
...
Generator.prototype.addEntityTranslationKey = function (key, value, language) {
    ...
    try {
        greencodeUtils.rewriteFile({
            file: fullPath,
            needle: 'greencode-needle-menu-add-entry',
            splicable: [
                '"' + key + '": "' + value + '",'
            ]
        }, this);
...
Generator.prototype.addSocialConfiguration = function (name, clientId, clientSecret, comment) {
    ...
    try {
        ...
        greencodeUtils.rewriteFile({
            file: fullPath,
            needle: 'greencode-needle-add-social-configuration',
            splicable: [
                config
            ]
        }, this);
...
Generator.prototype.addAngularJsModule = function (moduleName) {
    ...
    try {
        greencodeUtils.rewriteFile({
            file: fullPath,
            needle: 'greencode-needle-angularjs-add-module',
            splicable: [
                "'" + moduleName + "',"
            ]
        }, this);
    } catch (e) {
        this.log(chalk.yellow('\nUnable to find ') + fullPath + chalk.yellow(' or missing required greencode-needle. Reference to ') + moduleName + chalk.yellow(' not added to Greencode app.\n'));
    }
};
...
Generator.prototype.addAngularJsInterceptor = function (interceptorName) {
    ...
    try {
        greencodeUtils.rewriteFile({
            file: fullPath,
            needle: 'greencode-needle-angularjs-add-interceptor',
            splicable: [
                '$httpProvider.interceptors.push(\'' + interceptorName + '\');'
            ]
        }, this);
    } catch (e) {
        this.log(chalk.yellow('\nUnable to find ') + fullPath + chalk.yellow(' or missing required greencode-needle. Interceptor not added to Greencode app.\n'));
    }
};
...
Generator.prototype.addChangelogToLiquibase = function (changelogName) {
    try {
        ...
        greencodeUtils.rewriteFile({
            file: fullPath,
            needle: 'greencode-needle-liquibase-add-changelog',
            splicable: [
                '<include file="classpath:config/liquibase/changelog/' + changelogName + '.xml" relativeToChangelogFile="false"/>'
            ]
        }, this);
    } catch (e) {
        this.log(chalk.yellow('\nUnable to find ') + fullPath + chalk.yellow(' or missing required greencode-needle. Reference to ') + changelogName + '.xml ' + chalk.yellow('not added.\n'));
    }
};
...
Generator.prototype.addColumnToLiquibaseEntityChangeset = function (filePath, content) {
    try {
        greencodeUtils.rewriteFile({
            file: filePath,
            needle: 'greencode-needle-liquibase-add-column',
            splicable: [
                content
            ]
        }, this);
    } catch (e) {
        this.log(chalk.yellow('\nUnable to find ') + filePath + chalk.yellow(' or missing required greencode-needle. Column not added.\n') + e);
    }
};
...
Generator.prototype.addSocialButton = function (isUseSass, socialName, socialParameter, buttonColor, buttonHoverColor) {
    ...
    try {
        ...
        greencodeUtils.rewriteFile({
            ...
            needle: 'greencode-needle-add-social-button',
            splicable: [
                serviceCode
            ]
        }, this);
 ...
 * Add a new social connection factory in the SocialConfiguration.java file.
 *
 * @param {string} javaDir - default java directory of the project (Greencode var)
...
Generator.prototype.addSocialConnectionFactory = function (javaDir, importPackagePath, socialName, connectionFactoryClassName, configurationName) {
    ...
    try {
        ...
        greencodeUtils.rewriteFile({
            file: fullPath,
            needle: 'greencode-needle-add-social-connection-factory-import-package',
            splicable: [
                javaImport
            ]
        }, this);
        ...
        greencodeUtils.rewriteFile({
            file: fullPath,
            needle: 'greencode-needle-add-social-connection-factory',
            splicable: [
                javaCode
            ]
        }, this);
    } catch (e) {
        this.log(chalk.yellow('\nUnable to find ') + fullPath + chalk.yellow(' or missing required greencode-needle. Social connection ') + e + ' ' + chalk.yellow('not added.\n'));
    }
};
...
Generator.prototype.addMainCSSStyle = function (isUseSass, style, comment) {
    ...
    try {
        greencodeUtils.rewriteFile({
            file: fullPath,
            needle: 'greencode-needle-css-add-main',
            splicable: [
                styleBlock
            ]
        }, this);
    } catch (e) {
        this.log(chalk.yellow('\nUnable to find ') + fullPath + chalk.yellow(' or missing required greencode-needle. Style not added to Greencode app.\n'));
    }
};
...
Generator.prototype.addMainSCSSStyle = function (style, comment) {
    ...
    try {
        greencodeUtils.rewriteFile({
            file: fullPath,
            needle: 'greencode-needle-scss-add-main',
            splicable: [
                styleBlock
            ]
        }, this);
...
Generator.prototype.addMavenDependency = function (groupId, artifactId, version, other) {
    try {
        ...
        greencodeUtils.rewriteFile({
            file: fullPath,
            needle: 'greencode-needle-maven-add-dependency',
            splicable: [
                dependency
            ]
        }, this);
    } catch (e) {
        this.log(chalk.yellow('\nUnable to find ') + fullPath + chalk.yellow(' or missing required greencode-needle. Reference to ') + 'maven dependency (groupId: ' + groupId + ', artifactId:' + artifactId + ', version:' + version + ')' + chalk.yellow(' not added.\n'));
    }
};
...
Generator.prototype.addMavenPlugin = function (groupId, artifactId, version, other) {
    try {
        ...
        greencodeUtils.rewriteFile({
            file: fullPath,
            needle: 'greencode-needle-maven-add-plugin',
            splicable: [
                plugin
            ]
        }, this);
    } catch (e) {
        this.log(chalk.yellow('\nUnable to find ') + fullPath + chalk.yellow(' or missing required greencode-needle. Reference to ') + 'maven plugin (groupId: ' + groupId + ', artifactId:' + artifactId + ', version:' + version + ')' + chalk.yellow(' not added.\n'));
    }
};
...
Generator.prototype.addGradlePlugin = function (group, name, version) {
    try {
        ...
        greencodeUtils.rewriteFile({
            file: fullPath,
            needle: 'greencode-needle-gradle-buildscript-dependency',
            splicable: [
                'classpath group: \'' + group + '\', name: \'' + name + '\', version: \'' + version + '\''
            ]
        }, this);
    } catch (e) {
        this.log(chalk.yellow('\nUnable to find ') + fullPath + chalk.yellow(' or missing required greencode-needle. Reference to ') + 'classpath: ' + group + ':' + name + ':' + version + chalk.yellow(' not added.\n'));
    }
};
...
Generator.prototype.addGradleDependency = function (scope, group, name, version) {
    try {
        ...
        greencodeUtils.rewriteFile({
            file: fullPath,
            needle: 'greencode-needle-gradle-dependency',
            splicable: [
                scope + ' group: \'' + group + '\', name: \'' + name + '\', version: \'' + version + '\''
            ]
        }, this);
    } catch (e) {
        this.log(chalk.yellow('\nUnable to find ') + fullPath + chalk.yellow(' or missing required greencode-needle. Reference to ') + group + ':' + name + ':' + version + chalk.yellow(' not added.\n'));
    }
};
...
Generator.prototype.applyFromGradleScript = function (name) {
    try {
        ...
        greencodeUtils.rewriteFile({
            file: fullPath,
            needle: 'greencode-needle-gradle-apply-from',
            splicable: [
                'apply from: \'' + name + '.gradle\''
            ]
        }, this);
    } catch (e) {
        this.log(chalk.yellow('\nUnable to find ') + fullPath + chalk.yellow(' or missing required greencode-needle. Reference to ') + name + chalk.yellow(' not added.\n'));
    }
};
...
Generator.prototype.copyTemplate = function (source, dest, action, _this, _opt, template) {
    ...
    switch (action) {
        case 'stripHtml' :
            ...
            greencodeUtils.copyWebResource(source, dest, regex, 'html', _this, _opt, template);
            break;
        case 'stripJs' :
            var regex = /\,[\s\n ]*(resolve)\:[\s ]*[\{][\s\n ]*[a-zA-Z]+\:(\s)*\[[ \'a-zA-Z0-9\$\,\(\)\{\}\n\.\<\%\=\-\>\;\s]*\}\][\s\n ]*\}/g;
            //looks for something like mainTranslatePartialLoader: [*]
            greencodeUtils.copyWebResource(source, dest, regex, 'js', _this, _opt, template);
            break;
        case 'copy' :
...
/**
 * Register a module configuration to .greencode/modules/jhi-hooks.json
 *
 * @param {string} npmPackageName - npm package name of the generator
 * @param {string} hookFor - from which Greencode generator this should be hooked ( 'entity' or 'app')
...
Generator.prototype.registerModule = function (npmPackageName, hookFor, hookType, callbackSubGenerator, description) {
    try {
        var modules;
        var error, duplicate;
        var moduleName = _.startCase(npmPackageName.replace('generator-greencode-', ''));
        ...
        var moduleConfig = {
            ...
            description: description ? description : 'A Greencode module to generate ' + moduleName,
            hookFor: hookFor,
            hookType: hookType,
        ...
        if (shelljs.test('-f', MODULES_HOOK_FILE)) {
            ...
            try {
                ...
                error = true;
                this.log(chalk.red('The Greencode module configuration file could not be read!'));
            }
        } else {
        ...
        this.log('\n' + chalk.bold.red('Could not add greencode module configuration'));
    }
};
...
Generator.prototype.updateEntityConfig = function (file, key, value) {
    ...
    try {
        ...
        this.log(chalk.red('The Greencode entity configuration file could not be read!') + err);
    }
...
Generator.prototype.getExistingEntities = function (warn) {
    ...
    if (shelljs.test('-d', GREENCODE_CONFIG_DIR)) {
        shelljs.ls(path.join(GREENCODE_CONFIG_DIR, '*.json')).forEach(function (file) {
    ...
    if (entities.length != unique_dates.size) {
        this.log(chalk.yellow('WARNING some of your entities have the same changelog dates so Greencode couldn\'t\n' +
            ' determine the order in which they should be generated. It is recommended to\n' +
            ' edit the changelog dates in the ' + GREENCODE_CONFIG_DIR + 'folder and to relaunch this\n' +
            ' generator.'));
    }
...
Generator.prototype.updateLanguagesInLanguageConstant = function (languages) {
    ...
    try {
        ...
        content +=
            "            // greencode-needle-i18n-language-constant - Greencode will add/remove languages in this array\n" +
            "        ]";

        greencodeUtils.replaceContent({
            file: fullPath,
            pattern: /\.constant.*LANGUAGES.*\[([^\]]*greencode-needle-i18n-language-constant[^\]]*)\]/g,
            content: content
        }, this);
    } catch (e) {
        this.log(chalk.yellow('\nUnable to find ') + fullPath + chalk.yellow(' or missing required greencode-needle. LANGUAGE constant not updated with languages: ') + languages + chalk.yellow(' since block was not found. Check if you have enabled translation support.\n'));
    }
};
...
Generator.prototype.isGreencodeVersionLessThan = function (version) {
    var greencodeVersion = this.config.get('greencodeVersion');
    if (!greencodeVersion) {
        return true;
    }
    return semver.lt(greencodeVersion, version);
}

Generator.prototype.getDefaultAppName = function () {
    return (/^[a-zA-Z0-9_]+$/.test(path.basename(process.cwd()))) ? path.basename(process.cwd()) : 'greencode';
};
...
Generator.prototype.composeLanguagesSub = function (generator, configOptions, type) {
    if (generator.enableTranslation) {
        ...
        generator.composeWith('greencode:languages', {
            options: {
                'skip-install': true,
